...
static std::vector<short> vecDirectory[HASH_SIZE];
...
static std::vector<copydata_t> CpyData;
...
void InsertNodeDirectory(int start) // 8002D990
{
    ...
    if (VectorVersion) {
        vecDirectory[hashKey].push_back(start);
    }
    else {
...
void DeleteNodeDirectory(int start) // 8002DAD0
{
    ...
    if (VectorVersion) {
        for (auto it = vecDirectory[hashKey].begin(); it != vecDirectory[hashKey].end(); it++) {
            if (*it == start) {
                vecDirectory[hashKey].erase(it);
                break;
            }
        }
    }
...
int FindMatch(int start, int count) // 8002DC0C
{
    ...
    int encodedpos;
    int hashKey;
    std::vector<short>::reverse_iterator it;
    ...
    if (VectorVersion) {
        it = vecDirectory[hashKey].rbegin();
        offset = *it++;
    }
    else {
...
void DecodeD64(unsigned char* input, unsigned char* output) // 8002DFA0
{
    ...
    //printf("DecodeD64:Start\n");

    InitTables();

    OVERFLOW_READ = 0x7fffffff;
    OVERFLOW_WRITE = 0x7fffffff;
...
// ENCODE MODE BY GEC Eríck Vasquez García (ERICK194)
// Version vector, too slow when compressing large files like maps, but it works.

void EncodeCode(int lookup)
{
    int Code = lookup;
    short* evenTbl = &DecodeTable[0];
    short* oddTbl = &DecodeTable[629];
...
{
    int incrBit, cpyoff, vExtra, copyCnt, shiftPos;
    int cntFile, bufferEnd, LooKupCode, savecpy, readPos;
    int i, j, ret;
    VectorVersion = 1;
    ...
    for (i = 0; i < 63; i++) {
        if (GetReadSize() >= inputlen) { window[(readPos++) % windowSize] = 0; }
        else { window[(readPos++) % windowSize] = ReadByte(); }
    }

    ret = 1;
    bufferEnd = -1;
    while (cntFile < inputlen)
    {
        for (i = 0; i < ret; i++) {
            DeleteNodeDirectory(readPos);

            if (GetReadSize() >= inputlen) {
                if (bufferEnd == -1) { bufferEnd = readPos; }
                window[readPos] = 0;
            }
            else {window[readPos] = ReadByte();}
        ...
        InsertNodeDirectory(incrBit);

        ret = FindMatch(incrBit, windowSize);

        if ((bufferEnd != -1) && ((incrBit + ret) >= bufferEnd)) {
            break;
        }
        ...
        if (ret >= 3) {
            ...
            savecpy = 1;
            for (j = 0; j < (int)CpyData.size(); j++)
            {
                if ((cntFile > CpyData[j].offset) && (cntFile < (CpyData[j].offset + CpyData[j].cpycnt)))
                {
                    if (ret <= CpyData[j].cpycnt) {
                        savecpy = 0;
                    }
                    else if (ret > CpyData[j].cpycnt)
                    {
                        if ((cntFile - CpyData[j].offset) == 1) {
                            CpyData.erase(CpyData.begin() + j);
                        }
                        else {
                            savecpy = 0;
                        }
            ...
            if (savecpy) {
                cpyoff = (incrBit - encArgs.offset - ret);
                if (cpyoff < 0) {
                    
                    cpyoff += windowSize;
                }

                CpyData.push_back(copydata_t{ cntFile, ret, cpyoff , encArgs.offset });
            }
        }
        ret = 1;
    ...
    for (int i = 0; i < inputlen; i++)
    {
        ...
        for (int j = 0; j < (int)CpyData.size(); j++)
        {
            if (CpyData[j].offset == i)
            {
                copyCnt = CpyData[j].cpycnt;
                vExtra = CpyData[j].vExtra;
        ...
        if (copyCnt)
        {
            ...
            LooKupCode = 0;
            if (shiftPos == 0x04) { LooKupCode = (0x0376 + (count - 3)); }
            else if (shiftPos == 0x06) { LooKupCode = (0x03B4 + (count - 3)); }
            else if (shiftPos == 0x08) { LooKupCode = (0x03F2 + (count - 3)); }
            else if (shiftPos == 0x0A) { LooKupCode = (0x0430 + (count - 3)); }
            else if (shiftPos == 0x0C) { LooKupCode = (0x046E + (count - 3)); }
            else if (shiftPos == 0x0E) { LooKupCode = (0x04AC + (count - 3)); }

            EncodeCode(LooKupCode);

            auto it = binary.rbegin();
            for (int k = 0; k < (int)binary.size(); k++) {
                WriteBinary(*it++);
            }
            WriteCodeBinary(ValExtra, shiftPos);

            // Update Table
            UpdateTables(LooKupCode + -0x0275);

            i += copyCnt - 1;
        }
        else
        {
            LooKupCode = (input[i] + 0x0275);
            EncodeCode(LooKupCode);
...
unsigned char* EncodeD64(unsigned char* input, int inputlen, int* size) {
    ...
    hashtable = (short*)malloc(HASH_SIZE * sizeof(short));
    hashtarget = (short*)malloc(HASH_SIZE * sizeof(short));
    hashNext = (short*)malloc(windowSize * sizeof(short));
    hashPrev = (short*)malloc(windowSize * sizeof(short));
    memset(hashNext, 0, windowSize * sizeof(short));
    memset(hashPrev, 0, windowSize * sizeof(short));
